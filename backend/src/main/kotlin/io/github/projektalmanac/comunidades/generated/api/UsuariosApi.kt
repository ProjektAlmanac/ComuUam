...
interface UsuariosApi {
    ...
    @Operation(
        ...
        responses = [
            ApiResponse(responseCode = "200", description = "OK", content = [Content(schema = Schema(implementation = UsuarioCreadoDto::class))])
        ],
        security = [ SecurityRequirement(name = "jwt") ]
    )
    @RequestMapping(
            method = [RequestMethod.GET],
            value = ["/api/usuarios/me"],
            produces = ["application/json"]
    )
    fun getMe(): ResponseEntity<UsuarioCreadoDto>
}
